...
1.  Sometimes there are a few lines of metadata at the top of the file. You can
    ...
    read_csv("# A comment We want to skip
      x,y,z
      1,2,3", comment = "#")
    ```
...
1.  `parse_logical()` and `parse_integer()` parse logicals and integers
    ...
    parsers so We won't describe them here further.
...
*   `parse_datetime()` expects an ISO8601 date-time. ISO8601 is an
    ...
    dates and times frequently, We recommend reading
...
1.  We didn't discuss the `date_format` and `time_format` options to
