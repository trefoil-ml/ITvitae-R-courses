...
You can connect to `MySQL` and `MariaDB` (a Ffork of `MySQL`) using `src_mysql()`, mediated by the `RMySQL` package. Like `PostgreSQL`, you?ll need to provide a dbname, username, password, host, and port.
...
1. Read the csv files in the in the folder "AirOnTimeSmall" one for one  and store them in one large table called flights sqlite datbase. 
2. Collect the data from the airport, airlines and planes tables in the `nycflights13` package and copy them into the created database 
3. Retart R and connect to the created database 
4. Connect the to the created sqlite database (src_sqlite)
5. List table names for a database
6. Create reference tables  with `tbl`for the tables flights  airport, airlines and planes 
7. define an object called clean and contain data frrom the following from the table flights
   - not cancelled flights 
   - flights with dep_delay > 15, dep_delay < 240
   - data from the year 2006 
   - and only the columns year, arr_delay, dep_delay, distance, unique_carrier       
